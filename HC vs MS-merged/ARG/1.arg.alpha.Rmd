...
psr<-data.frame(richness=colSums(ps))

```

```{r}
psr$Sample <- rownames(psr)
```

```{r}
tab2 <- tab[, c("Sample", "Gene", "Group")]
```

```{r}
tab2$Group1[tab2$Group %in% "HC"] <- "HC"
tab2$Group1[!(tab2$Group %in% "HC")] <- "MS-Merged"
```
...
```{r}
theme_set(theme_bw())
p <- ggplot(d2, aes(x=Group1, y=richness, color=Group1)) + 
  geom_boxplot(alpha = 2, na.rm = TRUE, outlier.size = 0)+ 
                        theme(text = element_text(size = 13)) +
                        scale_colour_brewer(palette = "Set1") +
                        labs(y="Diversity richness", x = "Status")+
...
```{r}
#normality
shapiro.test(d2$richness)

#chao1
t1 <- wilcox.test(richness ~ Group1, data = d2,conf.int=TRUE,
       p.adjust.methods = "fdr",var.equal = TRUE)
t1

effect1 <- d2 %>% cohens_d(richness ~ Group1, var.equal = TRUE, hedges.correction = TRUE)
